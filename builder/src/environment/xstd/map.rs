...
pub fn register(env: &mut EnvironmentBuilder) {
    ...
    env.register_native_function("contains_key", Some(_type.clone()), vec![("key", key_type.clone())], contains_key, 10, Some(Type::Bool));
    env.register_native_function("get", Some(_type.clone()), vec![("key", key_type.clone())], get, 10, Some(Type::Optional(Box::new(value_type.clone()))));
    ...
    env.register_native_function("remove", Some(_type.clone()), vec![("key", key_type.clone())], remove, 10, Some(Type::Optional(Box::new(value_type.clone()))));
